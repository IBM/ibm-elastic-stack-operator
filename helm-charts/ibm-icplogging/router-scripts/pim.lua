...
function PIM:query_user_role(token, uid)
    ...
    local res, err = httpc:request_uri("https://platform-identity-management.kube-system.svc."..self.cluster_domain..":4500/identity/api/v1/users/" .. uid .. "/getHighestRoleForCRN", {
        method = "GET",
        ssl_verify = false,
        headers = {
...
function PIM:query_user_namespaces(token, uid)
    local httpc = http.new()
    res, err = httpc:request_uri("https://platform-identity-management.kube-system.svc."..self.cluster_domain..":4500/identity/api/v1/users/" .. uid .. "/getTeamResources", {
        method = "GET",
        ssl_verify = false,
        headers = {
          ["Content-Type"] = "application/json",
