...
BASE_DIR := ibm-elastic-stack-operator
...
ifeq ($(TARGET_OS),$(filter $(TARGET_OS),linux darwin))
	...
	@echo "Merging and push multi-arch image $(REGISTRY)/$(IMG):$(CSV_VERSION)"
...
install: ## Install all resources (CR/CRD's, RBCA and Operator)
	...
	- for crd in $(shell ls deploy/crds/*_crd.yaml); do kubectl apply -f $$crd --validate=false; done
	@echo ....... Applying RBAC .......
