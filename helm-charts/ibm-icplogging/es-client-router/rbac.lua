...
local function get_user_role(token, uid)
    local httpc = http.new()
    local res, err = httpc:request_uri("https://platform-identity-management.kube-system.svc."..cluster_domain..":4500/identity/api/v1/users/" .. uid .. "/getHighestRoleForCRN", {
        method = "GET",
        ssl_verify = false,
        headers = {
...
local function get_user_namespaces(token, uid)
    local httpc = http.new()
    res, err = httpc:request_uri("https://platform-identity-management.kube-system.svc."..cluster_domain..":4500/identity/api/v1/users/" .. uid .. "/getTeamResources", {
        method = "GET",
        ssl_verify = false,
        headers = {
          ["Content-Type"] = "application/json",
...
local function get_user_audit_namespaces(token, namespaces)
    ...
    res, err = httpc:request_uri("https://iam-pdp.kube-system.svc."..cluster_domain..":7998/v1/authz_bulk" , {
        method = "POST",
        ssl_verify = false,
        body = req_body,
        headers = {
